...
if($hasil['level'] == 4){
    ...
    $query = mysqli_query($conn, "SELECT DISTINCT o.id,  o.qty, o.notes, o.status, o.jam, o.tanggal, o.invoice_id, u.username, u.level, u.nama, u.nohp, u.alamat, d.nama_paket, d.keterangan, d.harga, SUM(d.harga*o.qty) AS harganya from tb_order o, tb_user u, tb_daftar_paket d WHERE o.user_id = u.id_user AND o.packet_id = d.id_paket GROUP BY o.invoice_id");
    while ($record = mysqli_fetch_array($query)) {
        $result[] = $record;
    }
}
...
<body style="background-color: #B4CDE6;">
    <div class="col-lg-9 mt-2">
        <div class="card" style="background: linear-gradient(to left, #82C3EC, #97DECE);">
            ...
            if (empty($result)) {
                echo "Data Paket Laundry Tidak Ditemukan";
            } else {
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <span class="badge rounded-pill text-bg-info">Dikirim</span>
    ...
    $('.add-one').click(function() {
        var element = $('.dynamic-element').first().clone().appendTo('.dynamic-stuff').end()
        $(element).find('select[name="layanan[]"]').attr('id', name)
        $(element).find('input[name="harga[]"]').attr('id', name)
        $(element).find('input[name=qty]').attr('id', name)
        name++;
        attach_delete();
    });
